...
export function expandRuntimeBundle(bundle) {
  ...
  const rules = (bundle.rules ?? []).map((entry, index) => {
    ...
    if (categoryTable[categoryIndex] === undefined) {
      throw new Error(`runtime bundle profile[${profileIndex}] references missing categoryTable[${categoryIndex}]`);
    }
    if (matchTable[matchIndex] === undefined) {
      throw new Error(`runtime bundle profile[${profileIndex}] references missing matchTable[${matchIndex}]`);
    }
    if (scopeTable[scopeIndex] === undefined) {
      throw new Error(`runtime bundle profile[${profileIndex}] references missing scopeTable[${scopeIndex}]`);
    }
    ...
    if (severityIndex !== undefined && severityTable[severityIndex] === undefined) {
      throw new Error(`runtime bundle profile[${profileIndex}] references missing severityTable[${severityIndex}]`);
    }
