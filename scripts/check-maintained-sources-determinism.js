...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nomsentry-maintained-sources-"));
...
try {
  ...
  const expectedFiles = fs.readdirSync(expectedDir).filter((entry) => entry.endsWith(".json")).sort();
  const candidateFiles = fs.readdirSync(candidateDir).filter((entry) => entry.endsWith(".json")).sort();

  if (JSON.stringify(expectedFiles) !== JSON.stringify(candidateFiles)) {
    throw new Error("Maintained source determinism check failed: rebuilt file set differs from custom/sources");
  }
  ...
  for (const file of expectedFiles) {
    ...
    if (expected !== actual) {
      throw new Error(`Maintained source determinism check failed: rebuilt ${file} differs from custom/sources/${file}`);
    }
  }
