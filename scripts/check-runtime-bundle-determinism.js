...
const expectedManifestFile = path.resolve(process.cwd(), "dist", "build-manifest.json");
...
try {
  ...
  writeProvenanceManifest(candidateManifestFile, buildProvenanceManifest({
    inputDir,
  ...
  if (expectedManifest !== actualManifest) {
    throw new Error("Runtime bundle determinism check failed: rebuilt manifest differs from dist/build-manifest.json");
  }
