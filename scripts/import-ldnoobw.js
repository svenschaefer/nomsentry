...
const REPO_API_URL =
  "https://api.github.com/repos/LDNOOBW/List-of-Dirty-Naughty-Obscene-and-Otherwise-Bad-Words/contents";
const RAW_BASE_URL =
...
export async function fetchAvailableLanguages(fetchImpl = fetch) {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch ${REPO_API_URL}: ${response.status} ${response.statusText}`,
    );
  }
