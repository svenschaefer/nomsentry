...
`reservedTechnical` is now sourced from Microsoft Windows reserved device names, a conservative Microsoft Windows reserved URI scheme subset, a conservative GitLab reserved-routes import, a conservative ICANN reserved-name subset, and a conservative filtered `reserved-usernames` import. That maintained v0.5 baseline explicitly covers route-collision, protocol-handler, registry, and system-identifier terms such as `root`, `system`, `mail`, `status`, `nic`, `whois`, and `settings`. Broader generic nouns such as `webhook`, `gateway`, `proxy`, `internal`, `private`, `public`, `service`, and `client` are intentionally left out of the maintained default profile because they are too noisy for the current default contract.

`impersonation` is now fed by the RFC 2142 core, a conservative GitHub Enterprise reserved-username import, a conservative additive `reserved-usernames` impersonation import, and a conservative derived additive layer in `custom/sources/derived-impersonation.json`. That maintained additive baseline now covers identifiers such as `staff`, `account`, `accounts`, `admin`, `administrator`, `billing`, `help`, `login`, `oauth`, `official`, `password`, `payment`, `payments`, `profile`, `reset`, `reset-password`, `secure`, `sysadmin`, and `webmail` without introducing a hand-maintained project wordlist. Broader marketing and trust-signaling nouns such as `verified`, `trust`, and `safety` remain intentionally outside the maintained default profile for v0.5 because no strong freely redistributable default source was identified for them.

`compositeRisk` is now fed by the RFC 2142 `security+support` rule plus a conservative derived support/security-anchor layer in `custom/sources/derived-composite-risk.json`. That layer covers exact-token combinations such as `admin-support`, `admin-security`, `billing-support`, `login-support`, `login-security`, `oauth-support`, `official-support`, `password-security`, `payment-support`, `profile-security`, and `reset-security`. Broader trust, privacy, verification, and recovery combinations such as `trust-safety`, `customer-recovery`, and `privacy-team` are intentionally outside the maintained default profile for v0.5.

The maintained v0.5 profanity split is now explicit and accepted for this phase: `insult.wiki` feeds `insult`, and `dsojevic/profanity-list` now maps `general` tagged entries to `generalProfanity`, `racial`, `religious`, and `lgbtq` tagged entries to `slur`, `sexual` tagged entries to `sexual`, and `shock` tagged entries to `shock` instead of leaving them all in the broad `profanity` bucket. This split remains intentionally source-based, so some overlapping terms can still surface both `profanity` and `generalProfanity` evidence, both `profanity` and `insult` evidence, both `profanity` and `slur` evidence, both `profanity` and `sexual` evidence, or both `profanity` and `shock` evidence. A separate `extremism` category is intentionally deferred because the current freely redistributable structured sources do not provide a clean maintained split axis for it.
