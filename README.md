...
The first explicit profanity split is now in place as well: `insult.wiki` feeds `insult` instead of the broad `profanity` bucket. This is intentionally source-based for now, so some overlapping terms can still surface both `profanity` and `insult` evidence until the broader category refinement is completed.
